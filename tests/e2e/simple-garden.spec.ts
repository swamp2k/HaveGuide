...
test('simple mobile navigation isolates tools and retries photo linking without duplicating the plant', async ({ page }) => {
  const errors: string[] = [];
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/media' && method === 'POST') { uploads++; return json({ media: { id: 'photo' } }, 201); }
    if (path.endsWith('/plants/plant/media')) {
      links++;
      if (links === 1) return json({ error: 'Forbindelsen blev afbrudt' }, 503);
  ...
  await page.locator('input[type=file]').first().setInputFiles({ name: 'leaf.png', mimeType: 'image/png', buffer: Buffer.from('iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAIAAACQkWg2AAAAGUlEQVR4nGN0arJnIAUwkaR6VMOohiGlAQBLagEjdrTcrAAAAABJRU5ErkJggg==', 'base64') });
  ...
  expect(creates).toBe(1); expect(uploads).toBe(1); expect(links).toBe(2); expect(identifyCalls).toBe(1);
  ...
  await page.screenshot({ path: 'test-results/simple-mobile.png', fullPage: true });
