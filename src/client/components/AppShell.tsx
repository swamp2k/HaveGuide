...
type Tab = 'garden' | 'media' | 'settings';
  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-content">
        ...
        {tab === 'settings' && (
          <SettingsPage user={user} garden={garden} onGardenChanged={onGardenChanged} onLogout={onLogout} />
        )}
      </div>

      <nav className="bottom-nav" aria-label="Hovednavigation">
        <button className={tab === 'garden' ? 'active' : ''} type="button" onClick={() => setTab('garden')} aria-current={tab === 'garden' ? 'page' : undefined}>
          <span aria-hidden="true">⌖</span><span>Min have</span>
        </button>
        <button className={tab === 'media' ? 'active' : ''} type="button" onClick={() => setTab('media')} aria-current={tab === 'media' ? 'page' : undefined}>
          <span aria-hidden="true">▧</span><span>Billeder</span>
        </button>
        <button className={tab === 'settings' ? 'active' : ''} type="button" onClick={() => setTab('settings')} aria-current={tab === 'settings' ? 'page' : undefined}>
          <span aria-hidden="true">⚙</span><span>Indstillinger</span>
        </button>
      </nav>
    </div>
  );
