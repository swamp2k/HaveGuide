        ...
        <>
          <div className="smart-scan-capabilities">
            ...
            <div className={capabilities.depthSupported ? 'ready' : 'optional'}>
              <span>Dybde</span><strong>{capabilityLabel(capabilities.depthSupported)}</strong>
            </div>
            <div className={capabilities.sceneSemanticsSupported ? 'ready' : 'optional'}>
              <span>Scene-forståelse</span><strong>{capabilityLabel(capabilities.sceneSemanticsSupported)}</strong>
            </div>
          </div>

          {capabilities.arCoreSupported && !capabilities.arCoreInstalled && (
            <button type="button" className="primary-button" disabled={busy} onClick={() => void prepareArCore()}>
              {busy ? 'Klargør…' : 'Klargør Smart Scan'}
            </button>
          )}

          {capabilities.arCoreInstalled && (
